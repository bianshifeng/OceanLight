...
typedef void*                        IMP_PVOID;
typedef unsigned long                IMP_SIZE_T;
typedef unsigned long                IMP_LENGTH_T;

typedef unsigned long                IMP_DWORD;
typedef unsigned long*               IMP_PDWORD;
typedef unsigned short               IMP_WORD;
typedef unsigned short*              IMP_PWORD;
typedef unsigned char                IMP_BYTE;
typedef long                         IMP_LONG;
...
#define imp_Round(val) ((int)((val) + ((val) >= 0 ? 0.5 : -0.5)))
...
{
	IMP_U32 u32MajorV;		//主版本号:
	IMP_U32 u32MinorV;		//子版本号:增加了功能时修改
	IMP_U32 u32RevisionV;	//修正版本号:修正了BUG时修改
}LIB_VERSION_S;
*/
...
typedef enum impTGT_EVENT_E
{
	...
	IMP_TGT_EVENT_ABNORMALVEL   = 0x00000080,	/**< 非正常速度 */
	IMP_TGT_EVENT_LOITER        = 0x00000100,	/**< 徘徊 */
	IMP_TGT_EVENT_OSC		    = 0x00000200,	/**< OSC遗弃物检测 */
	IMP_TGT_EVENT_OSC_R		    = 0x00000400,	/**< OSC被盗物检测 */
}TGT_EVENT_E;
...
typedef enum impEVT_TYPE_E
{
	...
	IMP_EVT_TYPE_AlarmOsc		    = 0x0000010a,		/**< OSC遗弃物检测 */
	IMP_EVT_TYPE_AlarmOsc_R		    = 0x0000010b,		/**< OSC被盗物检测 */
...
{
    IMP_S32 s32SceneChg;            /**< 场景改变 */
    IMP_S32 s32SignalValue;	        /**< 信号值 */
	IMP_S32 s32ClarityValue;        /**< 模糊度值 */
	IMP_S32 s32BrightHValue;        /**< 亮度高值 */
	IMP_S32 s32BrightLValue;        /**< 亮度低值 */
	IMP_S32 s32ColorValue;	        /**< 颜色值 */
	IMP_S32 s32NoiseValue;	        /**< 噪声值 */
	IMP_S32 s32PtzValue;            /**< PTZ失控 */
	IMP_S32 s32FreezValue;          /**< 画面冻结 */
	IMP_S32 s32InterfereValue;      /**< 人为干扰 */
}AVD_STATCL_DATA_S;

/** AVD 处理结果 */
typedef struct impAVD_RESULT_S
{
    AVD_STATCL_DATA_S stStatclData;	/**< AVD统计数据 */
	EVT_SET_S	stEventSet;			/**< AVD报警数据 */
}AVD_RESULT_S;
