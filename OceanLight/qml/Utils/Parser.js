...
//^  表示开头
//$  表示结尾
var REGEXP_IMG =/(\w+\.(?:png|jpg|gif|bmp))$/;
...
function parseAlarm(itemDB){
    var tmp = itemDB
    tmp.alarmId = tmp.alarmId
    tmp.alarmName = tmp.alarmName
    tmp.alarmType = tmp.alarmType
    tmp.alarmLevel = tmp.alarmLevel
    tmp.alarmTime = tmp.alarmTime
    tmp.alarmDevId = tmp.alarmDevId
    tmp.alarmDevName = tmp.alarmDevName
    tmp.alarmContent =tmp.alarmContent
    tmp.alarmImgUrl = tmp.alarmImgUrl


    tmp.isFavourited = (tmp.isFavourited === 1 ? true : false)
    tmp.isHandled =(tmp.isHandled === 1 ? true : false)
    tmp.isDeleted =(tmp.isDeleted === 1 ? true : false)
...
function paserTimeToDate(time){
    return timeToDate(parseInt(time),"");
}

function  parseImgPathImgName(path){
    var m_path = path.toString() || '';
    console.debug("---path" + m_path)
    var xxx ="";
    if(m_path){
        xxx =m_path.match(REGEXP_IMG)[0];
        xxx = xxx.split('.')[0];
    }
    return xxx;
}

function parserIPtoRTSP(ipAddress){

//    return "rtsp://"+ipAddress+":8554/0"
    return "rtsp://"+ipAddress+":554/0"
